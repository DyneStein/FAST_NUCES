...
#simpler implementation for C++
def single_point_crossover_manual(parent_a, parent_b):
...
#C++
def two_point_crossover_manual(parent_a, parent_b):
    ...
    # part 1: copy left of A
    for i in range(0, cut1):
        child.append(parent_a[i])

    # part 2: copy middle of B
    for i in range(cut1, cut2):
        child.append(parent_b[i])

    # part 3: copy right of A
...
#c++
def order_crossover_manual(parent_a, parent_b):
    ...
    # create child with placeholder
    child = []
    for i in range(size):
        child.append(None)

    # STEP 1 copy segment from A
    for i in range(cut1, cut2+1):
        child[i] = parent_a[i]

    # STEP 2 store used cities
    used = set()
    for i in range(cut1, cut2+1):
        used.add(parent_a[i])
